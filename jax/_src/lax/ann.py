...
def _approx_top_k_abstract_eval(operand, *, k, reduction_dimension,
  ...
  if not dtypes.issubdtype(operand.element_type, np.floating):
...
def _get_init_val_literal(op_type, is_max_k):
  if is_max_k:
    if dtypes.issubdtype(op_type, np.floating):
      return np.array(np.NINF, dtype=op_type)
    else:
      return np.iinfo(op_type).min
  else:
    if dtypes.issubdtype(op_type, np.floating):
      return np.array(np.Inf, dtype=op_type)
    else:
      return np.iinfo(op_type).max
